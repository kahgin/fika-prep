...
-- Create trigram index for admin areas (simplified without unaccent)
...
LANGUAGE sql STABLE AS $$
  SELECT DISTINCT ON (a.id)
    ...
    CASE 
      ...
      -- For planning_area: Show "Planning Area, Singapore"
      -- WHEN a.kind = 'planning_area' THEN a.name || ', ' || get_country_name(a.country_iso2)
      -- For state: Show "State, Country" (e.g., "Johor, Malaysia")
  ...
  WHERE
    ...
    -- Optional: Add kind filter to exclude/include specific types
    -- AND a.kind IN ('country', 'state')  -- Exclude planning_area when commented out
    -- AND a.kind IN ('country', 'state', 'planning_area')  -- Include planning_area when enabled
  ORDER BY
    a.id,
...
-- Alternative version WITH planning area support (currently commented out)
/*
CREATE OR REPLACE FUNCTION rpc_search_locations(
  p_query text,
  p_limit int DEFAULT 5
...
LANGUAGE sql STABLE AS $$
  ...
  WHERE
    lower(a.name) % lower(p_query)
    OR lower(a.name) LIKE lower(p_query) || '%'
    -- Include all kinds when planning areas are enabled
    AND a.kind IN ('country', 'state', 'planning_area')
  ORDER BY
    a.id,
    CASE WHEN lower(a.name) LIKE lower(p_query) || '%'
         THEN 0 ELSE 1 END,
